...
export const Template16 = ({ data, pageData }: TemplateProps) => {
    const { content, isFirstPage, isLastPage, pageIndex } = pageData;
    // Light gray/beige sidebar background
    const sidebarBg = '#e5e5e5';

    return (
        <div className={`flex-1 text-[#1a1b3a] text-sm leading-relaxed font-serif overflow-hidden flex flex-row`}>

            {/* Left Sidebar - Light Gray - Only on Page 1 */}
            {isFirstPage && (
                <div className="w-[35%] p-8 flex flex-col flex-shrink-0" style={{ backgroundColor: sidebarBg }}>
                    {/* Name - Stacked and Bold */}
                    <h1 className="text-4xl font-extrabold text-gray-900 mb-2 tracking-tight leading-tight break-words w-full font-serif">
                        {data.personal.fullName}
                    </h1>

                    {/* Job Title */}
                    <p className="text-lg text-gray-700 mb-8 font-medium break-words w-full">
                    ...
                    {data.personal.photo && (
                        <div className="mb-8">
                            <img
                                ...
                                alt="Profile"
                                className="w-28 h-28 rounded-full object-cover grayscale-[20%]"
                            />
                        </div>
                    )}
                    ...
                    <div className="flex flex-col gap-3 text-xs font-semibold text-gray-700">
                        ...
                        {data.personal.address && (
                            <div className="flex items-start gap-2 min-w-0">
                                <MapPin className="w-3.5 h-3.5 flex-shrink-0 text-gray-900 mt-0.5" />
            ...
            <div className={`flex-1 p-12 flex flex-col min-w-0 ${!isFirstPage ? 'pt-16' : ''}`}>
                ...
                {/* Signature - Layout: Sincerely Left, Block Right - Only on the LAST page */}
                {isLastPage && (
