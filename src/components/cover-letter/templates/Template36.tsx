...
export const Template36 = ({ data, pageData }: TemplateProps) => {
    const { content, isFirstPage, isLastPage, pageIndex } = pageData;
    // Colors
    const nameColor = '#000000';
    const accentLineColor = '#a8d5e2'; // Light sky blue/cyan from image
    ...
    return (
        <div className={`flex-1 text-[#333] text-sm leading-relaxed font-sans overflow-hidden flex flex-col p-12 relative`}>
            ...
            {isFirstPage && (
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex-1 pr-8 min-w-0">
                        ...
                        <p className="text-sm font-bold text-black break-words uppercase tracking-wide">
                            {data.personal.jobTitle}
                        </p>
                    </div>

                    {/* Right: Contact Info (Text only as per design) */}
                    <div className="flex flex-col items-end gap-1 text-[11px] text-gray-500 font-normal pt-1 flex-shrink-0 text-right">
                        {data.personal.email && (
            ...
            {/* Thick Accent Line - Only on Page 1 */}
            {isFirstPage && (
            ...
            <div className={`flex-1 relative z-10 ${!isFirstPage ? 'pt-16' : ''}`}>
                {/* Recipient with "TO" Badge - Only on Page 1 */}
                {isFirstPage && (
                    <div className="mb-8 flex items-start gap-3">
                        <div className="px-2 py-0.5 text-[10px] font-bold tracking-wider rounded-sm flex-shrink-0 mt-0.5" style={{ backgroundColor: toBadgeBg, color: toBadgeText }}>
                            TO
                        </div>
                        <div className="text-sm text-black font-bold break-words leading-relaxed">
                            <span>{data.recipient.name}</span>
                            {data.recipient.company && <span className="font-normal text-gray-800">, {data.recipient.company}</span>}
                        </div>
                    </div>
                )}

                {/* Subject / Re: Line (Simulated as it's common in this template style) - Only on Page 1 */}
                {isFirstPage && (
                    <div className="mb-6 font-bold text-sm text-black">
