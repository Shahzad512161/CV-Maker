...
export const Template6 = ({ data, pageData }: TemplateProps) => {
    const { content, isFirstPage, isLastPage, pageIndex } = pageData;

    return (
        <div className={`flex-1 text-[#1a1b3a] text-sm leading-relaxed font-sans flex overflow-hidden`}>
            {/* Left Sidebar - Dark Purple - Only on Page 1 */}
            {isFirstPage && (
                <div className="w-[38%] bg-[#581c4d] text-white p-8 flex flex-col flex-shrink-0">
                    <h1 className="text-3xl font-bold mb-1 tracking-tight leading-tight break-words w-full">
                        {data.personal.fullName}
                    </h1>
                    <p className="text-lg font-medium text-white/80 mb-8 break-words w-full">
                    ...
                    <div className="flex flex-col gap-3 text-xs font-medium text-white/90">
                        {data.personal.email && (
                    ...
                    {data.personal.socials.length > 0 && (
                        <div className="flex flex-col gap-3 text-xs font-medium text-white/90 mt-3">
